...
int BP_init(unsigned btbSize, unsigned historySize, unsigned tagSize, unsigned fsmState,
	...
	int HISTS_NUM;
	if(isGlobalTable){
		COUNTERS_NUM = pow(2,historySize);
		Predictor_Table->tables_type = GLOBAL_TABLES;
	}
	else{
		COUNTERS_NUM = btbSize;
		Predictor_Table->tables_type = LOCAL_TABLES;
	}
	Predictor_Table->FSM = (bool**)malloc(sizeof(bool*)*(COUNTERS_NUM));
	...
	for(unsigned i=0 ;i<COUNTERS_NUM ;i++){
		Predictor_Table->FSM[i] = (bool*)malloc(sizeof(bool)*2);
		...
		decimalToBinary(fsmState,Predictor_Table->FSM[i]);
	}
	//history
	if(isGlobalHist){
		HISTS_NUM = 1;
	...
	if(found){
		*dst = Predictor_Table->target[index];
		...
		if(Predictor_Table->Shared == using_share_lsb){
			...
			return Predictor_Table->FSM[fsm_index][0];
		}
...
void BP_update(uint32_t pc, uint32_t targetPc, bool taken, uint32_t pred_dst){
	...
	if(!Predictor_Table->valid[index] || !found){
		...
		decimalToBinary(NewState, Predictor_Table->FSM[fsm_index]);
	}
